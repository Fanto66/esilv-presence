...
class Cours:
    def __init__(self, utilisateur, identifiant, heure_debut, heure_fin):
    ...
    def type_appel(self):
        ...
        if deja_present.count() > 0 and deja_present.is_visible(timeout=0):
            logging.info(f"Le cours {self.identifiant} a déjà été noté présent pour {self.utilisateur.email}.")
            return "deja_present"
        ...
        if bouton_validation.count() > 0:
            logging.info(f"Le cours {self.identifiant} est ouvert pour {self.utilisateur.email}.")
            return "open"
